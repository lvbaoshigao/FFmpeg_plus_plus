...
/// 快捷配置编辑器 —— 全屏左右分栏，左侧列配置项，右侧编辑参数。
/// 通过 Navigator.push + MaterialPageRoute 打开，onSave 回调保存更新后的配置。
...
class _QuickConfigPageState extends State<QuickConfigPage> {
  ...
  /// 左面板每项下方的简要参数描述（如 "Bitrate (kbps): 2000 · CRF: 23"）。
  String _itemSummary(QuickConfigItem item) {
    final parts = <String>[];
    item.params.forEach((k, v) {
      final label = _humanizeKey(k);
      if (v is bool) {
        parts.add('$label: ${v ? 'on' : 'off'}');
      } else if (v is num) {
        parts.add('$label: $v');
      } else if (v is String && v.isNotEmpty) {
        final display = v.length > 20 ? '${v.substring(0, 20)}...' : v;
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: _fileTypeColor(_config.fileType, scheme).withAlpha(80),
                      ),
                    ),
                    child: Text(
                      ...
                      style: TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w600,
                        color: _fileTypeColor(_config.fileType, scheme),
                      ),
                    ),
                  ),
                ],
  ...
  Widget _buildHorizontalLayout(ColorScheme scheme, bool isZh) {
    return Row(
      ...
      children: [
        ...
        SizedBox(
          width: 280,
          child: _buildLeftPanel(scheme, isZh),
        ),
        VerticalDivider(width: 1, color: scheme.outlineVariant.withAlpha(60)),
        // 右面板
        Expanded(
          child: _buildRightPanel(scheme, isZh),
        ),
      ],
    );
  }
          ...
          Expanded(
            child: _config.items.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) =>
                        _buildItemTile(_config.items[i], i, scheme, isZh),
                  ),
          ),
        ],
    );
  ...
  Widget _buildItemTile(QuickConfigItem item, int index, ColorScheme scheme, bool isZh) {
    ...
    final summary = _itemSummary(item);
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: item.enabled
                      ...
                      : scheme.surfaceContainerHighest.withAlpha(80),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              isZh ? '请从左侧选择一个配置项' : 'Select an item from the left',
          ...
          if (item.params.isEmpty)
            ...
            ...item.params.entries.map((entry) {
              return Padding(
                ...
                child: _buildParamField(item.key, entry.key, entry.value, scheme, isZh),
              );
            }),
        ],
      ),
  ...
  Widget _buildParamField(
    ...
    final label = _paramLabel(paramKey);
