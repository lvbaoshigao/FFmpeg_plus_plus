...
class GlassPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 透明度：0.1~1.0，映射到背景 alpha
    final op = cfg.cardOpacity.clamp(0.1, 1.0);
    ...
    if (effect == 'none') {
      // 无效果：纯色卡片 + 圆角 + 细边框（透明度跟随）
      return RepaintBoundary(
        child: Container(
          padding: padding,
          decoration: BoxDecoration(
            color: baseColor.withAlpha(((isDark ? 235 : 245) * op).round()),
            borderRadius: br,
      ...
      child: DecoratedBox(
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: _LiquidGlassBody(borderRadius: br, child: glassBody),
          ),
        ),
      ),
    );
...
class _LiquidGlassBody extends StatelessWidget {
  ...
  final Widget child;
  const _LiquidGlassBody({required this.borderRadius, required this.child});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final edge = Paint()
      ..shader = SweepGradient(
        ...
        colors: [
          Colors.white.withValues(alpha: 0.52), // 顶部高光
          Colors.white.withValues(alpha: 0.30), // 右上
          Colors.black.withValues(alpha: 0.12), // 底部阴影
          Colors.black.withValues(alpha: 0.05), // 左下
          Colors.white.withValues(alpha: 0.52), // 回到顶部
        ],
